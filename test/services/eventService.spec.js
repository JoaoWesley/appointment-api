...
describe('Event service', () => {
  ...
  it('Should try save and return EVENT_ALREADY_EXISTS exception', async () => {
    ...
    try {
      await eventService.createEvent('2019-10-10')
    } catch (error) {
  ...
  it('checkIfOutSideAvailableHours - Should Return false', () => {
    ...
    const result = eventService.checkIfOutSideAvailableHours(
      dateTimeWithConfigTimeZone.format()
    )
  ...
  it('checkIfOutSideAvailableHours - Should Return true', () => {
    ...
    dateTimeWithConfigTimeZone = moment
      .utc(dateTimeWithConfigTimeZone)
      .subtract(appointmentConfig.DURATION_IN_MINUTES, 'minutes')
  ...
  it('checkIfIsInvalidTimeForSlot - Should Return false', () => {
    ...
    dateTimeWithConfigTimeZone = moment
      .utc(dateTimeWithConfigTimeZone)
      .add(appointmentConfig.DURATION_IN_MINUTES, 'minutes')

    const result = eventService.checkIfIsInvalidTimeForSlot(
      dateTimeWithConfigTimeZone.format()
    )
    expect(result).to.equal(false)
  })
  ...
  it('checkIfIsInvalidTimeForSlot - Should Return false', () => {
    ...
    dateTimeWithConfigTimeZone = moment
      .utc(dateTimeWithConfigTimeZone)
      .add(appointmentConfig.DURATION_IN_MINUTES + 3, 'minutes')
